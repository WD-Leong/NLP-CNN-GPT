...
class MultiHeadAttention(torch.nn.Module):
    def __init__(
        self, d_model, n_heads, ker_sz, neg_infty=-1.0e9):
        ...
        # For the CNN Unfolding. #
        self.unfold = torch.nn.Unfold(
            (ker_sz, 1), dilation=1, padding=0, stride=1)
        
        # Linear Mappings. #
        self.wc = torch.nn.Linear(
            ker_sz*d_model, d_model, bias=False)
        self.wq = torch.nn.Linear(
            ker_sz*d_model, ker_sz*d_model, bias=False)
    ...
    def forward(self, v, k, q, mask=None):
        ...
        zero_pad = torch.zeros(
            batch_sz, self.ker_sz-1, 
            self.d_model, dtype=torch.float32)
        
        if torch.cuda.is_available():
            zero_pad = zero_pad.to("cuda")
        
        q_pad = torch.cat((zero_pad, q), dim=1)
        k_pad = torch.cat((zero_pad, k), dim=1)
        v_pad = torch.cat((zero_pad, v), dim=1)
        
        q_pad = torch.unsqueeze(
            torch.transpose(q_pad, 2, 1), 3)
        k_pad = torch.unsqueeze(
            torch.transpose(k_pad, 2, 1), 3)
        v_pad = torch.unsqueeze(
            torch.transpose(v_pad, 2, 1), 3)
        
        q_pad = torch.transpose(
            self.unfold(q_pad), 2, 1)
        k_pad = torch.transpose(
            self.unfold(k_pad), 2, 1)
        v_pad = torch.transpose(
            self.unfold(v_pad), 2, 1)
        
        q_input = self.split_heads(self.wq(q_pad))
        k_input = self.split_heads(self.wk(k_pad))
        v_input = self.split_heads(self.wv(v_pad))
...
class DecoderLayer(torch.nn.Module):
    def __init__(
        self, d_model, n_heads, d_ffwd, ker_sz, 
        ...
        self.attn_self = MultiHeadAttention(
            d_model, n_heads, ker_sz, neg_infty=neg_infty)
...
class Decoder(torch.nn.Module):
    def __init__(
        ...
        d_model, d_ffwd, vocab_size, ker_sz, 
        ...
        for m in range(n_layers):
            ...
            self.dec_layers.append(DecoderLayer(
                d_model, n_heads, d_ffwd, ker_sz, rate1, rate2))
...
class GPTDecoder(torch.nn.Module):
    def __init__(
        ...
        max_seq_length, ker_sz=3, rate1=0.1, rate2=0.1):
